...
// Table cell/head idiom reused from finance-page for the md+ ledger table.
...
/** TOPUP and COMMISSION_REFUND are credits (green, +); COMMISSION_DEBIT is a debit (rose, −). */
function isCreditTx(type: OrgWalletTxRow['type']): boolean {
  return type === 'TOPUP' || type === 'COMMISSION_REFUND';
}

/** Uzbek label for a ledger row's type. */
function txTypeLabel(type: OrgWalletTxRow['type']): string {
  if (type === 'TOPUP') return "To'ldirish";
  if (type === 'COMMISSION_REFUND') return 'Komissiya qaytarildi';
...
/**
 * The three summary tiles derived client-side from the ledger. `amountSom` is a
 ...
 */
function WalletKpis({ transactions }: { transactions: OrgWalletTxRow[] }) {
  let toppedUp = 0n;
  let commission = 0n;
  for (const tx of transactions) {
    const amount = BigInt(tx.amountSom);
    if (tx.type === 'TOPUP') toppedUp += amount;
    else if (tx.type === 'COMMISSION_DEBIT') commission += amount;
    else if (tx.type === 'COMMISSION_REFUND') commission -= amount;
  }
  ...
  return (
    <StatTileRow className="grid grid-cols-2 gap-3 sm:grid-cols-3">
      <StatTile
        ...
        tone="green"
      />
      <StatTile label="Jami komissiya" value={formatPriceSom(commission.toString(), 'SALE')} />
 ...
 * a real table (Sana / Turi / Miqdor + a plain-text fixation reference cell). The
 * fixation cell is a LABEL, not a link — the CRM app has no fixation route.
 */
function LedgerSection({
  transactions,
  className,
}: {
  transactions: OrgWalletTxRow[];
  className?: string;
}) {
  return (
    <section className={className}>
      ...
      {transactions.length === 0 ? (
        ...
        <>
          ...
          {/* md+: real table reusing the finance-page table styles. */}
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="border-b border-line">
                    ...
                    <th className={HEAD}>Fiksatsiya</th>
                  </tr>
                </thead>
                <tbody>
                  {transactions.map((tx) => (
 ...
 * comes from `type` — credits are green (+), COMMISSION_DEBIT a rose debit (−).
 */
...
/** One ledger row as a table row (md+). The fixation cell is plain text, never a link. */
function WalletTxRow({ tx }: { tx: OrgWalletTxRow }) {
  const isCredit = isCreditTx(tx.type);
  const amount = formatPriceSom(tx.amountSom, 'SALE');
  ...
  return (
    <tr className="border-b border-line">
      ...
      <td className={cn(CELL, 'text-ink-3')}>
        {tx.fixationId ? `Komissiya · ${tx.fixationId.slice(0, 8)}` : '—'}
      </td>
    </tr>
  );
}
...
function TopupPanel({
  ...
  topup: ReturnType<typeof useOrgTopup>;
  className?: string;
}) {
  return (
    <section className={className}>
