...
/**
 * Shown instead of the create / "my requests" pages when `useSession()` reports
 * no logged-in user. A local copy of the my-listings prompt — the FSD boundary
 * rule forbids a `pages → pages` import, so the tiny pattern is duplicated with
 * board-specific copy.
 */
  ...
  return (
    <div className="flex min-h-[60dvh] flex-col items-center justify-center gap-4 px-6 text-center">
      <span className="flex h-14 w-14 items-center justify-center rounded-full bg-accent-soft text-accent">
        <Icon name="search" className="h-6 w-6" strokeWidth={2.1} />
      </span>

      <div>
        <h1 className="text-[19px] font-extrabold tracking-tight">Kirish qiling</h1>
        <p className="mt-1.5 max-w-[320px] text-[14px] text-ink-2">
          Qidiruv joylash va o'z so'rovlaringizni boshqarish uchun avval hisobingizga kiring.
        </p>
      </div>
