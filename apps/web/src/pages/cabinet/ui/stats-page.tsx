...
/** /cabinet/obj/:id/stats (design spec §4.3, §8.3) — a dedicated compact page rather
...
export function StatsPage() {
  ...
  // signed-out visitor landing here directly must not fire doomed authenticated
  // requests.
  const enabled = !meLoading && Boolean(realtor);
  const listing = useQuery({ ...myListingQuery(id), enabled });
  const stats = useQuery({ ...myObjectStatsQuery(id), enabled });
  ...
  // the sign-in prompt (same convention as EditListingPage/ProfilePage).
  ...
  const error = listing.error ?? stats.error;
  if (error) {
