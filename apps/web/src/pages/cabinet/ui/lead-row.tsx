 ...
 *  unparseable value is shown as-is rather than crashing the row. `monthLabel` looks
 *  up the cabinet ns `month.0`..`month.11` keys — passed in rather than calling
 *  useTranslation() here, since this is a plain function, not a hook. */
function formatLeadDate(iso: string, monthLabel: (monthIndex: number) => string): string {
  const date = new Date(iso);
  if (Number.isNaN(date.getTime())) return iso;
  const hh = String(date.getHours()).padStart(2, '0');
  const mm = String(date.getMinutes()).padStart(2, '0');
  return `${date.getDate()}-${monthLabel(date.getMonth())}, ${hh}:${mm}`;
}
  ...
  return (
    <div className="rounded-card border border-line/60 bg-card p-3.5 shadow-card">
      ...
      <p className="mt-2 truncate text-[12.5px] font-semibold text-ink-3">
        {listingTitle ?? t('listingFallbackTitle')} ·{' '}
        {formatLeadDate(lead.createdAt, (monthIndex) => t(`month.${monthIndex}`))}
      </p>
      ...
      {targets.length > 0 && (
        <div className="mt-2.5 flex flex-wrap gap-2 border-t border-line/60 pt-2.5">
          {targets.map((to) => (
            ...
            >
              {LEAD_TRANSITION_LABEL[to] ? t(LEAD_TRANSITION_LABEL[to]) : to}
            </button>
          ))}
        </div>
      )}
      ...
      {advance.isError && (
        <p className="mt-2 text-[12.5px] font-bold text-red-600">{t('statusChangeError')}</p>
      )}
    </div>
  );
}
