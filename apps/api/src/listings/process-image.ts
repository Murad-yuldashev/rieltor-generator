...
/**
 * Shared by the seed pipeline (prisma/seed.ts, via the prisma/images.ts
 * re-export) and the guarded upload endpoint (listing-image.controller.ts) —
 * this is the one place sharp is invoked, so both paths produce identical
 * variants.
 */
