...
import { CollectionSummaryCard } from './collection-summary';
 ...
 *
 * On phone it is a single stack (header → present/share → items), preserved via the
 * `contents` wrappers + `order-*`. From `lg` it becomes a main column (header + items)
 * beside a sticky aside (summary + present/share). Layout only — every mutation and
 * behaviour is unchanged from the pre-desktop version.
 */
    ...
    <main>
      ...
      {isPending ? (
        ...
        <div className="flex flex-col gap-5 lg:grid lg:grid-cols-[1fr_22rem] lg:items-start lg:gap-6 desk:grid-cols-[1fr_24rem]">
          {/* MAIN column — the stateful header + the items grid (or empty CTA). The
              `contents` wrapper dissolves on phone so its children join the single
              stack, ordered by `order-*`. */}
            ...
            {collection.items.length === 0 ? (
              <div className="order-3 rounded-card bg-card p-8 text-center shadow-card lg:order-none">
                <p className="text-[15px] font-bold text-ink">
                  Bu kolleksiya bo'sh — e'lon qo'shing
                </p>
                <Link
                  to="/browse"
          ...
          {/* STICKY ASIDE — DOM order summary-first so the desktop rail shows the summary
              on top; `order-*` keeps the phone order (present/share right under the header,
              summary appended last). */}
